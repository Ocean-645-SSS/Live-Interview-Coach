...
TTS 多久产生第一段音频
用户说完到助手开始播音的总延迟
Agent 到 LiveKit Server 的 TCP 网络延迟
"""
...
class MetricsState:
    ...
    last_user_created_at:float|None=None    #最近一条用户消息产生的时间
    last_user_stop_at:float|None=None   #用户最近停止说话的时间
    last_interim_text:str=""    #STT最近一次中间转录
    last_tcp_rtt_ms:float|None=None    #最近一次TCP网络延迟
