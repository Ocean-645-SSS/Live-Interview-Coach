...
class ContextStore:
    ...
    def read_session_system_prompt(self,session_id:str)->str:
        ...
        return self._read_txt(self._session_paths(session_id).system_prompt_file,"")
    

    def write_session_system_prompt(self,session_id:str,content:str)->None:
        """写入本次通话已经锁定的系统提示词。"""

        self._session_paths(session_id).system_prompt_file.write_text(content.rstrip() +"\n",encoding="utf-8")
    ...
    @staticmethod
    def _read_txt(path:Path,default:str)->str:
        """读取指定文件文档"""
