    ...
    def initialize(self):
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (DEFAULT_KB_ID, DEFAULT_KB_NAME, "", now, now),
            )
    ...
    def create_knowledge_base(
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                values (?, ?, ?, ?, ?)
                """,
                (kb_id,clean_name,clean_description,now,now)
            )
        
        #创建知识库目录（测试使用）
        kb_dir = self.knowledge_bases_dir / kb_id
...
#============document相关操作========================
    ...
    def update_document_index_status(self,kb_id:str,document_id:str,*,index_status:str,chunks_count:int=None,error_msg:str=None):
        """更新文档索引状态"""
